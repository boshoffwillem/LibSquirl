...
public class GroupsApiTests
{
    ...
    public async Task CreateAsync_SendsCorrectRequest()
    {
        ...
        Group result = await api.CreateAsync(
            new CreateGroupRequest { Name = "my-group", Location = "aws-us-east-1" }
        );
    ...
    public async Task GetAsync_SendsCorrectRequest()
    {
        ...
        Assert.Equal(HttpMethod.Get, handler.Requests[0].Method);
        Assert.Equal(
            $"/v1/organizations/{OrgSlug}/groups/default",
    ...
    public async Task AddLocationAsync_SendsCorrectRequest()
    {
        ...
        Assert.Equal(HttpMethod.Post, handler.Requests[0].Method);
        Assert.Equal(
            $"/v1/organizations/{OrgSlug}/groups/default/locations/aws-eu-west-1",
            handler.Requests[0].Uri.AbsolutePath
        );
    }
    ...
    public async Task CreateTokenAsync_WithParams_SendsCorrectRequest()
    {
        ...
        TokenResponse result = await api.CreateTokenAsync("default", "2w", "read-only");
    ...
    public async Task InvalidateTokensAsync_SendsCorrectRequest()
    {
        ...
        Assert.Equal(HttpMethod.Post, handler.Requests[0].Method);
        Assert.Equal(
            $"/v1/organizations/{OrgSlug}/groups/default/auth/rotate",
            handler.Requests[0].Uri.AbsolutePath
        );
    }
    ...
    public async Task GetConfigurationAsync_SendsCorrectRequest()
    {
        ...
        handler.EnqueueResponse(
            HttpStatusCode.OK,
            """{"heartbeat_url":"https://example.com/hb","allow_attach":true}"""
        );

        GroupConfiguration result = await api.GetConfigurationAsync("default");

        Assert.Equal("https://example.com/hb", result.HeartbeatUrl);
        Assert.True(result.AllowAttach);
        Assert.Equal(HttpMethod.Get, handler.Requests[0].Method);
        Assert.Equal(
            $"/v1/organizations/{OrgSlug}/groups/default/configuration",
            handler.Requests[0].Uri.AbsolutePath
        );
    }
    ...
    public async Task UpdateConfigurationAsync_SendsCorrectRequest()
    {
        ...
        handler.EnqueueResponse(
            HttpStatusCode.OK,
            """{"heartbeat_url":"https://example.com/hb2","allow_attach":false}"""
        );

        GroupConfiguration result = await api.UpdateConfigurationAsync(
            "default",
            new UpdateGroupConfigurationRequest { HeartbeatUrl = "https://example.com/hb2" }
        );
        ...
        JsonDocument body = JsonDocument.Parse(handler.Requests[0].Body!);
        Assert.Equal(
            "https://example.com/hb2",
            body.RootElement.GetProperty("heartbeat_url").GetString()
        );
    }
    ...
    public async Task TransferAsync_SendsCorrectRequest()
    {
        ...
        await api.TransferAsync("default", new TransferGroupRequest { Organization = "other-org" });

        Assert.Equal(HttpMethod.Post, handler.Requests[0].Method);
        Assert.Equal(
            $"/v1/organizations/{OrgSlug}/groups/default/transfer",
            handler.Requests[0].Uri.AbsolutePath
        );
    ...
    public async Task UnarchiveAsync_SendsCorrectRequest()
    {
        ...
        Assert.Equal(HttpMethod.Post, handler.Requests[0].Method);
        Assert.Equal(
            $"/v1/organizations/{OrgSlug}/groups/default/unarchive",
            handler.Requests[0].Uri.AbsolutePath
        );
    }
}
