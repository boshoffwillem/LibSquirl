...
public interface ILibSqlClient
{
    ...
    Task<BatchResult> BatchAsync(Batch batch, CancellationToken cancellationToken = default);

    /// Executes a sequence of semicolon-separated SQL statements.
    Task SequenceAsync(string sql, CancellationToken cancellationToken = default);

    /// Describes (parses/analyzes) a SQL statement.
    Task<DescribeResult> DescribeAsync(string sql, CancellationToken cancellationToken = default);
    ...
    Task<MigrationJobsSummary> ListMigrationJobsAsync(
        CancellationToken cancellationToken = default
    );

    /// Gets details about a specific migration job.
    Task<MigrationJobDetail> GetMigrationJobAsync(
        int jobId,
        CancellationToken cancellationToken = default
    );
}
