...
public sealed class LocationsApi : PlatformApiBase, ILocationsApi
{
    ...
    public LocationsApi(
        ...
        TursoPlatformOptions options,
        HttpClient? regionHttpClient = null
    )
        : base(httpClient, options)
    {
        _regionHttpClient =
            regionHttpClient ?? new HttpClient { BaseAddress = new Uri("https://region.turso.io") };
    }
    ...
    {
        LocationsWrapper wrapper = await GetAsync<LocationsWrapper>(
            "/v1/locations",
            cancellationToken
        );
    ...
    {
        return (
            await _regionHttpClient.GetFromJsonAsync<ClosestRegion>(
                "/",
                JsonOptions,
                cancellationToken
            )
